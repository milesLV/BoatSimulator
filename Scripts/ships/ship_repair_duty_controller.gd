...
func assign_crewmate(crewmate: Crewmate, decks: Array = []) -> bool:
	...
	ship.crew_task_controller.prepare_for_repair_duty(crewmate)
...
func queue_next_action(crewmate: Crewmate, replace_current := false) -> bool:
	...
	ship.crew_task_controller.queue_repair_actions(crewmate, plan["actions"], replace_current)
...
func plan_next_repair_step(crewmate: Crewmate) -> Dictionary:
	...
	var bail_actions = ship.action_planner.build_bail_water(crewmate, true)
...
func _build_next_damage_control_plan(crewmate: Crewmate) -> Dictionary:
	...
	if crewmate.bucket_amount > 0.0:
		...
		var carried_bucket_actions = ship.action_planner.build_bail_water(crewmate, true)
	...
	if not entry_candidates.is_empty():
		...
		var flooded_entry_actions = ship.action_planner.build_flooded_mid_deck_entry_bail(
			crewmate,
	...
	var safety_bail_actions = ship.action_planner.build_bail_water(crewmate, true)
...
func _build_repair_hole_plan(crewmate: Crewmate) -> Dictionary:
	...
	for hole in candidate_holes:
		var actions = ship.action_planner.build_repair_hole(crewmate, hole)
...
## Quickest trip first, with the name breaking ties so the order is stable.
func _get_sorted_repair_targets(crewmate: Crewmate, require_safe: bool) -> Array:
	...
	for hole in ship.action_points.hull_holes:
		...
		var repair_trip = ship.action_planner.estimate_repair_trip(crewmate, hole)

		if repair_trip["total_time"] == INF:
			continue

		if require_safe and not ship.action_planner.is_repair_trip_safe(crewmate, hole, repair_trip, flood_rate):
			continue
...
func _get_bail_rate(crewmate: Crewmate) -> float:
	...
	return Crewmate.MAX_BUCKET_AMOUNT / ship.action_planner.estimate_repair_bail_cycle_duration(crewmate)
...
func _get_flood_rate_without(crewmate: Crewmate) -> float:
	...
	for other in active_crewmates:
		if other != crewmate and not hole_by_crewmate.has(other) and other.action_executor.has_actions():
			flood_rate = maxf(flood_rate - _get_bail_rate(other), 0.0)

	return flood_rate


## Printed once per stretch of bailing while the water still gains.
...
func _has_damaged_holes(unreserved_only := false) -> bool:
	...
	return ship.action_points.hull_holes.any(
		func(hole):
			return (
