...
func _process_wheel(delta: float) -> void:
	...
	if station_controller != null and station_controller.get_operator_by_name(&"Wheel") != null:
		wheel_rotation += (
			turn_input
			* WHEEL_TURN_SPEED
			* delta
...
func _process_sail_rotation(delta: float) -> void:
	...
	sail.rotation = clamp(sail.rotation, 0.0, 2.0 * BASE_SAIL_ANGLE)


func _anchor_holding() -> bool:

	return anchor_system != null and anchor_system.is_holding_ship
