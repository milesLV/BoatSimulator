...
public class SkeleShip extends Boat {
     ...
     * 1. Chasing - if player is far away, go full speed towards them (aim to the left of player.getX() so don't actually clip, make variable for the distance)
     * 2. Attacking - if in range, slow down and try to circle them while firing (if too close, back up (perhaps according to variable ^))
     ...
     * 2.5 Circle of death - if get mast down, slow down to mid sail and circle them, make a function out of this 
     ...
     *  Reward system for getting the player as much in the triangle as possible
     */
}
