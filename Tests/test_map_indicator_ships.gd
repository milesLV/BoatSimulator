...
# Map indicators against the real map: lifecycle, cleanup, hull measurement.
# The pure geometry lives in test_map_indicators.gd.
...
func _run() -> void:
	...
	# every ship owns a distinct cull mask, and its cannon cones stay off it
	var masks := {}
	...
	for ship in map.ships:
		...
		assert(ship.get_parent().visibility_layer & mask != 0)  # ancestors carry the bit

		for cannon in ship.cannons:
			assert(cannon.visibility_layer == mask)
			assert(cannon.get_node("CannonRange").visibility_layer == 1)

	# --- both boats off screen: indicators wake up inside the padded border ---
	...
	for ship in map.ships:
		...
		# the boat sits on the medallion, the arrow orbits it at its authored radius
		...
		# 50k units away is far past the fade distance
		assert(is_equal_approx(live["arrow"].modulate.a, 1.0))
		# the render camera tracks the ship it belongs to
		assert(live["camera"].global_position == ship.global_position)

	# --- camera back on the player: only that indicator stands down ---
	# the two ships start close together, so sail the enemy off before looking
	...
	# --- a sunk-and-freed ship takes its indicator and render target with it ---
	...
	assert(not is_instance_valid(orphaned_viewport))   # no leaked render target
	assert(not is_instance_valid(orphaned_medallion))

	# --- hull radius comes from the collision polygon, with a fallback ---
